...
* The `FindById` method is doing some extra work to populate the `Id` field of the `User` object it is returning.
...
****
*  *Strong Consistency* - Core operations like Updates/Save/FindById are strongly consistent.
...
\http://localhost:5000/api/listTenantUsers?tenantId=0
...
Note: You may need to view a document in the bucket, or edit a document to have the appropriate values to satisfy the query. 
\http://localhost:5000/api/findActiveUsersByFirstName?firstName=David&enabled=false&countryCode=DE
...
Note: You may need to view or edit the documents to get the appropriate values for the query below.
\http://localhost:5000/api/ftsListActiveUsers?firstName=David&enabled=false&countryCode=DE
